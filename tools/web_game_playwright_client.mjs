...
async function captureCanvasPngBase64(canvas) {
  return canvas.evaluate((target) => {
    ...
    return commaIndex === -1 ? "" : data.slice(commaIndex + 1);
  });
}
...
async function isCanvasTransparent(canvas) {
  ...
  return canvas.evaluate((target) => {
    try {
      ...
      probeContext.drawImage(target, 0, 0, size, size);
      const imageData = probeContext.getImageData(0, 0, size, size).data;
...
async function main() {
  ...
  for (let i = 0; i < args.iterations; i++) {
    ...
    const screenshotPath = path.join(args.screenshotDir, `shot-${i}.png`);
